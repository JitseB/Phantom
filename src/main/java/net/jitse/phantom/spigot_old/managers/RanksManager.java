...
import net.jitse.phantom.spigot_old.account.rank.PhantomRank;
...
public class RanksManager {
    ...
    public boolean load(Config ranksConfig) {
        ...
        for (String name : section) {
            if (name.length() > 16) {
                SpigotLogger.log(plugin, SpigotLogger.LogLevel.WARN, "Tried to load rank " + name + ", but it has an invalid " +
        ...
        SpigotLogger.log(plugin, SpigotLogger.LogLevel.DEBUG, "Loaded " + ranks.size() + " ranks from ranks.yml.");

        this.defaultRank = getRank(plugin.getSettingsConfig().getString("DefaultRank")).orElse(null);
        if (defaultRank == null) {
            SpigotLogger.log(plugin, SpigotLogger.LogLevel.FATAL, "Default rank does not exist is the ranks.yml.");
            return false;
        }

        SpigotLogger.log(plugin, SpigotLogger.LogLevel.DEBUG, "Set \"" + defaultRank.getName() + "\" as the default rank.");
        return true;
    }
